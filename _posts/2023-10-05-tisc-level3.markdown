...
The main part of the mobile app is found in `com > tisc.kappa > MainActivity`. In it, the method that pertains to the flag can immediately be found:
...
A statement that logs the return value of the `css()` method is added like so, on line 11:
