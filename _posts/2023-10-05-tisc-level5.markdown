...
However, it only contained 2 commits and only the `.github/workflows`, which is normally used to hold GitHub Action workflow files.

![](/assets/images/tisc2023/Pasted image 20231001210149.png)

Lets open the only file in this repository.
...
On this page, there was only one failed run called "Portal opening". In it was a failed step, which had rror logs which revealed something interesting.
...
The Discord bot has access to 3 different text channels, one of which seemed to might hold the flag. To create their contents, the script is modified to print the message history of all 3 channels:
...
In one of the text channels was a conversation between 2 persons. There was also a mention of a `client_id` and the value `1076936873106231447`, which seemed to maybe belong to a member on the Discord server. Thus, the script is modified to print the identity of this member:
